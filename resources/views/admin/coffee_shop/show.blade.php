                ...
                <h1>
                    ...
                    <span id="page-actions">
                        @if($coffeeShop->status === 'requested')
                            <a href="{{route('admin.coffee_shop.show', ['coffee_shop' => $previous])}}"
                               class="btn btn-default @if($previous->id === $coffeeShop->id) disabled @endif">Previous</a>
                            <a href="{{route('admin.coffee_shop.review', ['coffee_shop' => $coffeeShop, 'status' => 'accepted'])}}"
                               class="btn btn-success"
                               data-confirm="Have you called them to verify it was them?">Accept</a>
                            <a href="{{route('admin.coffee_shop.review', ['coffee_shop' => $coffeeShop, 'status' => 'declined'])}}"
                               class="btn btn-warning">Decline</a>
                            <a href="{{route('admin.coffee_shop.show', ['coffee_shop' => $next])}}"
                               class="btn btn-default @if($next->id === $coffeeShop->id) disabled @endif">Next</a>
                        @else
                            <a href="{{route('admin.coffee_shop.review', ['coffee_shop' => $coffeeShop, 'status' => 'requested'])}}"
