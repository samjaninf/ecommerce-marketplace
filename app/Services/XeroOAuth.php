...
class XeroOAuth
{
     ...
     * @var array
     */
    private $_xero_defaults;

    /**
     * @var array
     */
    private $_xero_consumer_options;
    ...
    function __construct($config)
    {
        ...
        $this->config =
            array_merge($this->_xero_defaults, $this->_xero_consumer_options, $this->_xero_curl_options, $config);
    }
     ...
     *
     * @return int the string length of the header
     */
     ...
     *
     * @return mixed response data is stored in the class variable 'response'
     */
    private function curlit()
    {
        ...
        // configure curl
        $c         = curl_init();
        $useragent = ( empty ( $this->config['user_agent'] ) ) ? 'XeroOAuth-PHP' : $this->config['user_agent'];
        curl_setopt_array($c, [
     ...
     * @return
     */
    function request($method, $url, $params = [], $xml = "", $format = 'xml')
    {
        ...
        if ($this->response['code'] == 403) {
            $errorMessage =
                "It looks like your Xero Entrust cert issued by Xero is either invalid or has expired. See http://developer.xero.com/api-overview/http-response-codes/#403 for more";
    ...
    {
        $this->request('GET', $this->url('AccessToken', ''), [
    ...
    function diagnostics()
    {
        ...
        if ($this->config['application_type'] == 'Partner') {
            if ( ! file_get_contents($this->config['curl_ssl_cert'])) {
                $testOutput['ssl_cert_error'] =
                    "Can't read the Xero Entrust cert. You need one for partner API applications. http://developer.xero.com/documentation/getting-started/partner-applications/ \n";
            } else {
                $data      = openssl_x509_parse(file_get_contents($this->config['curl_ssl_cert']));
                $validFrom = date('Y-m-d H:i:s', $data['validFrom_time_t']);
                if (time() < $data['validFrom_time_t']) {
                    $testOutput['ssl_cert_error'] =
                        "Xero Entrust cert not yet valid - cert valid from " . $validFrom . "\n";
                }
                $validTo = date('Y-m-d H:i:s', $data['validTo_time_t']);
                if (time() > $data['validTo_time_t']) {
                    $testOutput['ssl_cert_error'] = "Xero Entrust cert expired - cert valid to " . $validTo . "\n";
                }
            }
        }
        ...
        if ($this->config['application_type'] == 'Partner' || $this->config['application_type'] == 'Private') {
            ...
            if ( ! file_exists($this->config['rsa_public_key'])) {
                $testOutput['rsa_cert_error'] =
                    "Can't read the self-signed SSL cert. Private and Partner API applications require a self-signed X509 cert http://developer.xero.com/documentation/advanced-docs/public-private-keypair/ \n";
            }
            if (file_exists($this->config['rsa_public_key'])) {
                ...
                if (time() < $data['validFrom_time_t']) {
                    $testOutput['ssl_cert_error'] =
                        "Application cert not yet valid - cert valid from " . $validFrom . "\n";
                }
                $validTo = date('Y-m-d H:i:s', $data['validTo_time_t']);
                if (time() > $data['validTo_time_t']) {
                    $testOutput['ssl_cert_error'] = "Application cert cert expired - cert valid to " . $validTo . "\n";
                }
            }
            if ( ! file_exists($this->config['rsa_private_key'])) {
                $testOutput['rsa_cert_error'] =
                    "Can't read the self-signed cert key. Check your rsa_private_key config variable. Private and Partner API applications require a self-signed X509 cert http://developer.xero.com/documentation/advanced-docs/public-private-keypair/ \n";
            }
            if (file_exists($this->config['rsa_private_key'])) {
